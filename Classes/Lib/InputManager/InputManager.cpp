...
void CInputFlag::up( kInputType keyType_ ) {
	...
	switch( keyType_ ) {
		...
		case kInputType::LEFT_SHIFT: this->m_left_shift = true; break;
...
void CInputFlag::down( kInputType keyType_ ) {
	switch( keyType_ ) {
		...
		case kInputType::LEFT_SHIFT: this->m_left_shift = false; break;
...
bool CInputFlag::isKeyPressed( kInputType keyType_ ) {
	...
	switch( keyType_ ) {
		...
		case kInputType::LEFT_SHIFT: return this->m_left_shift; break;
...
kInputType CInputManager::changeToInputTypeFromKeyCode( cocos2d::EventKeyboard::KeyCode keyCode_ ) {
	...
	// LEFT_SHIFTキーかどうか判定
	else if (cocos2d::EventKeyboard::KeyCode::KEY_LEFT_SHIFT == keyCode_)
	{
		// LEFT_SHIFT キーを返す
		return kInputType::LEFT_SHIFT;
	}
